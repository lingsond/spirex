...
def parse_args():
    ...
    parser.add_argument('--model', type=str, help="The model ['flant5', 'unifiedqa']", required=True, default='flant5')
    parser.add_argument('--question', type=str, help="Question version ['0', '1', '2']", required=True)
    parser.add_argument('--prompt', type=str, help="Prompt version ['1', '2']", required=True, default='1')
...
def run_inference(mdl, qver, pver):
    ...
    if mdl == 'google/flan-t5-large':
        cfile = f'sim_context_flant5_q{qver}_p{pver}.jsonl'
    else:
        cfile = f'sim_context_unifiedqa_q{qver}_p{pver}.jsonl'
    qlist = get_json_list(qfile)
    clist = get_json_list(cfile)
    predictions = predict(flist, qlist, clist, pver)
    if mdl == 'google/flan-t5-large':
        output_file = f'tg_flant5_q{qver}_p{pver}.jsonl'
    else:
        output_file = f'tg_unifiedqa_q{qver}_p{pver}.jsonl'
...
if __name__ == '__main__':
    ...
    if args.model == 'flant5':
        ...
        model = 'marksverdhei/unifiedqa-large-reddit-syac'
        TOKENIZER = AutoTokenizer.from_pretrained(model, cache_dir=CACHE_DIR)
        MODEL = AutoModelForSeq2SeqLM.from_pretrained(model, cache_dir=CACHE_DIR)

        MODEL = MODEL.cuda()
        run_inference(model, args.question, args.prompt)
