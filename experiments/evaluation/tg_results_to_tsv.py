...
def get_result(mdl, qver, pver):
    # input_file = f'output_{mdl}_q{qver}_p{pver}.txt'
    input_file = f'baseline_{mdl}_q{qver}_p{pver}.txt'
...
if __name__ == '__main__':
    ...
    for model in MODELS:
        for qver in qvers:
            for pver in pvers:
                result = get_result(model, qver, pver)
                results.append(result)
    with open('baseline_tg.tsv', 'w') as fh:
    # with open('output_tg.tsv', 'w') as fh:
        for item in results:
            fh.writelines(item + '\n')
