...
def check_context(jfile, mdl, qversion, pversion, qtag):
    cfile = f'sim_context_{mdl}_q{qversion}_p{pversion}.jsonl'
    jlist = get_json_list(jfile)
    clist = get_json_list(cfile)

    print(f'Statistics for TG Model = {mdl} - Qver: {qversion} - Pver: {pversion} - {qtag} only')
...
if __name__ == '__main__':
    ...
    for tag in tags:
        for model in models:
            for qver in qvers:
                for pver in pvers:
                    check_context(output2, model, qver, pver, tag)
